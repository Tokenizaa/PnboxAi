...
import { BusinessTemplate } from '../automation/businessTemplates';
import { DeepResearchReport } from '../types/pnbox';
...
export const BUSINESS_ARCHETYPES: BusinessArchetype[] = [
  ...
  },
  {
    ...
    descricaoPadrao: 'Agência focada em aquisição de clientes, tráfego pago, SEO e estruturação de funis de vendas B2B com foco in ROI comprovado.',
    ticketMedio: 'alto'
  },
  {
    id: 'energia_solar',
...
export class SchemaGenerator {
   ...
   * ATENÇÃO: Este método ainda gera dados mock para compatibilidade com funcionalidades existentes.
   * Para operações reais, use generateFromResearch em vez disso.
   */
  ...
  /**
   * Wrapper compatível para código legado que chamava gerarTodosOsSchemas.
   * Mapeia parâmetros antigos para GenerateMockOptions e retorna as 14 coleções.
   * ATENÇÃO: Gera dados mock - para uso real, prefira generateFromResearch.
   * explicitamenteGenerateMock: true indica que é uso intencional para demo/teste.
   */
  public static gerarTodosOsSchemas(
    businessInfo: {
      nomeEmpresa?: string;
      setor?: string;
    ...
    return this.generateMockData({
      idPlano,
      companyName: businessInfo.nomeEmpresa,
      businessSector: businessInfo.setor,
      businessDescription: businessInfo.resumoExecutivo,
      randomVariance: 0.2,
      explicitlyGenerateMock: true
    });
  }
   ...
   * ATENÇÃO: Este método SÓ gera dados mock se explicitlyGenerateMock === true.
   * Para operações reais, use generateFromResearch em vez disso.
   * Por padrão retorna arrays vazios para evitar dados fictícios em produção.
   */
  public static generateMockData(
    options: GenerateMockOptions = {}
  ): Record<string, Record<string, unknown>[]> {
    // SEGURANÇA: Não gerar dados mock a menos que explicitamente solicitado para demo/teste
    if (!options.explicitlyGenerateMock) {
      console.warn('[SchemaGenerator] generateMockData chamado sem explicitlyGenerateMock=true. Retornando arrays vazios para evitar dados fictícios em produção.');
    ...
    // Ferramentas complementares opcionals
  ...
  private static generateSegmentacaoMercado(
    ...
    const pool = {
      ...
      cafeteria_gastronomia: [
        {
          descricao: 'Professionals Remotos e Nômades Digitais',
      ...
      saude_odontologia: [
        ...
        },
        {
          ...
          variavel2Oposto: 'Consultos esporádicos avulses',
  ...
  private static generateFunilVendas(
    ...
    return itens.slice(0, count).map((item) => ({
      ...
      qtdPessoasChamdas: this.applyVariance(item.qtdPessoasChamdas, variance)
    }));
  }
