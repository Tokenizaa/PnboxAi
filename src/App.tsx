...
const initialSession:AuthSessionState={status:'idle',cpf:'',idPlano:'',modoExecucao:'LIVE',logs:[{timestamp:new Date().toISOString(),mensagem:'Painel inicializado. Aguardando sessão PNBOX real.',level:'info'}]};
...
export function App(){
 ...
 const [plans,setPlans]=useState<PlanoCriadoInfo[]>([]); const [session,setSession]=useState<AuthSessionState>(initialSession);
 ...
 const load=useCallback(async()=>{try{const h=headers();const a=await fetch('/api/automation/auth/status',{headers:h});if(a.ok){const d=await a.json();if(d.session)setSession(d.session);}const r=await fetch('/api/pnbox/plans',{headers:h});if(r.ok){const d=await r.json();if(Array.isArray(d.planos)){const remote=d.planos as PlanoCriadoInfo[];setPlans(remote);setActiveId(c=>remote.some(p=>p.idPlano===c)?c:(remote[0]?.idPlano||''));}}const t=await fetch('/api/automation/traffic',{headers:h});if(t.ok){const d=await t.json();if(Array.isArray(d.eventos))setTraffic(d.eventos);}}catch(e){console.warn('[App] falha ao carregar dados PNBOX',e);}},[]);
 useEffect(()=>{void load();},[load]);
 const active=plans.find(p=>p.idPlano===activeId); const tool=FERRAMENTAS_PNBOX.find(f=>f.id===toolId);
 const selectPlan=(id:string)=>{const real=id.trim();if(!real||real===':idPlano'){notify({level:'error',title:'Plano inválido',message:'Selecione um plano real retornado pelo PNBOX.'});return;}if(!plans.some(p=>p.idPlano===real)){notify({level:'error',title:'Plano não localizado',message:'O ID selecionado não pertence à lista retornada pelo PNBOX.'});return;}setActiveId(real);setSession(s=>({...s,idPlano:real,modoExecucao:'LIVE'}));setView('tools_matrix');};
 const syncPlans=async()=>{setSyncing(true);try{const r=await fetch('/api/pnbox/plans',{headers:headers()});const d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(d.message||'Falha ao consultar PNBOX.');const remote=Array.isArray(d.planos)?d.planos as PlanoCriadoInfo[]:[];setPlans(remote);setActiveId(c=>remote.some(p=>p.idPlano===c)?c:(remote[0]?.idPlano||''));notify({level:'success',title:'PNBOX sincronizado',message:`${remote.length} plano(s) oficial(is) carregado(s).`});}catch(e){notify({level:'error',title:'Falha na sincronização',message:e instanceof Error?e.message:'Erro desconhecido.'});}finally{setSyncing(false);}};
 const updateActive=(updater:(plan:PlanoCriadoInfo)=>PlanoCriadoInfo)=>{setPlans(current=>current.map(p=>p.idPlano===activeId?updater(p):p));};
 const runAi=async()=>{if(!active)return;setGenerating(true);try{const r=await fetch('/api/ai/deep-research',{method:'POST',headers:headers(true),body:JSON.stringify({prompt:`${active.nomePlano} - ${active.descricao}`,ideiaNegocio:`${active.nomePlano} - ${active.descricao}`,cidadeUf:active.cidadeUf,provider:'gemini'})});const rd=await r.json();if(!r.ok||!rd.report)throw new Error(rd.mensagem||'Pesquisa IA sem relatório.');const s=await fetch('/api/ai/synthesize-plan',{method:'POST',headers:headers(true),body:JSON.stringify({research:rd.report,idPlano:active.idPlano})});const sd=await s.json();if(!s.ok)throw new Error(sd.mensagem||'Falha na síntese IA.');const data=sd.dados14Ferramentas||sd.planData;if(!data||typeof data!=='object')throw new Error('A IA não retornou dados das ferramentas.');updateActive(p=>({...p,dados14Ferramentas:data,ferramentasPreenchidas:Object.values(data).filter((x:any)=>Array.isArray(x)&&x.length).length}));notify({level:'success',title:'IA concluída',message:'Dados sintetizados para o plano PNBOX real.'});}catch(e){notify({level:'error',title:'Erro na IA',message:e instanceof Error?e.message:'Erro desconhecido.'});}finally{setGenerating(false);}};
 const syncAll=async()=>{if(!active)return;setSyncing(true);try{const r=await fetch('/api/automation/fill-batch',{method:'POST',headers:headers(true),body:JSON.stringify({templateId:'cafeteria_coworking',idPlano:active.idPlano,dados:active.dados14Ferramentas,modoExecucao:'LIVE'})});const d=await r.json().catch(()=>({}));if(!r.ok&&r.status!==207)throw new Error(d.mensagem||d.message||'Falha na sincronização.');notify({level:d.status==='partial'?'warn':'success',title:d.status==='partial'?'Sincronização parcial':'Sincronização concluída',message:`Registros confirmados: ${d.data?.totalRegistrosSalvos||0}.`});await load();}catch(e){notify({level:'error',title:'Falha ao gravar no PNBOX',message:e instanceof Error?e.message:'Erro desconhecido.'});}finally{setSyncing(false);}};
 const saveItems=(items:Record<string,unknown>[])=>{if(!active||!tool)return;updateActive(p=>({...p,dados14Ferramentas:{...(p.dados14Ferramentas||{}),[tool.id]:items}}));};
 const applyCopilot=(id:string,data:Record<string,unknown>[])=>{if(active)updateActive(p=>({...p,dados14Ferramentas:{...(p.dados14Ferramentas||{}),[id]:data}}));};
 const createPlan=(p:PlanoCriadoInfo)=>{if(!p.idPlano||p.idPlano.startsWith('plano_')||p.idPlano===':idPlano'){notify({level:'error',title:'Criação não confirmada',message:'O PNBOX não retornou um ID real.'});return;}setPlans(x=>[...x,p]);setActiveId(p.idPlano);setView('tools_matrix');};
