...
pnboxPlansRouter.post('/plans/:id/tools/:ferramentaId/item', authMiddleware, async (req, res) => {
  const userId = (req as any).user.id; const { id: idPlano, ferramentaId } = req.params;
  try { const { authContext } = requirePnboxSession(userId); const resultado = await salvarOuAtualizarRegistroFerramentaPnbox(ferramentaId, req.body || {}, idPlano, authContext); return resultado.confirmed ? res.json({ status: 'ok', resultado }) : res.status(502).json({ status: 'error', code: 'PNBOX_TOOL_SAVE_UNCONFIRMED', resultado }); }
  catch (err: any) { return res.status(502).json({ status: 'error', message: err.message }); }
});
