...
await page.fill('input[name="username"], input#username', cpf);
    ...
    // Marcar "Lembre-se de mim" se houver e estiver visível
    const rememberCheckbox = page.locator('input[name="rememberMe"]');
    const rememberCount = await rememberCheckbox.count();
    if (rememberCount > 0) {
      // Usar isVisible() antes de check - check() explode se não visível
      const isVis = await rememberCheckbox.first().isVisible().catch(() => false);
      if (isVis) {
        await rememberCheckbox.first().check().catch((err: any) => {
          console.error('[OIDC/Playwright] Failed to check rememberMe checkbox:', err.message);
        });
      } else {
        console.log('[OIDC/Playwright] rememberMe checkbox encontrado mas não visível — pulando');
      }
    }
...
await page.waitForLoadState('networkidle', { timeout: 15000 }).catch((err: any) => {
    ...
    while (Date.now() - startExtractTime < EXTRACT_TIMEOUT_MS) {
const extracted = await page.evaluate(() => {
      ...
      if (extracted?.loginToken && extracted?.userId) {
        console.log(`[OIDC/Playwright] Meteor tokens found: userId=${extracted.userId}, tokenLen=${extracted.loginToken?.length || 0}`);
        meteorData = extracted;
        break;
      }

      // Log progresso a cada 5s
      const elapsed = Date.now() - startExtractTime;
      if (elapsed % 5000 < 1000) {
        console.log(`[OIDC/Playwright] Aguardando tokens Meteor... ${elapsed/1000}s decorridos, url=${page.url()}`);
      }

// Recarregar se Meteor ainda não inicializou (SPA pode precisar de reload)
       if (elapsed > 12000 && !/pnbox\.sebrae\.com\.br\/?$/.test(page.url())) {
...
if (!meteorData.loginToken || !meteorData.userId) {
        ...
        console.error(`[OIDC/Playwright] Token Meteor não encontrado após ${EXTRACT_TIMEOUT_MS/1000}s. URL: ${page.url()}. Screenshot: ${debugPath}`);
